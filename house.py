...
from get_random import generate_house
...
sum_word = len(house['word'].unique())
for i in range(sum_word): 
    word_to_lookup = generate_house()
    ...
    if dictionary_data is not None:
        if not dictionary_data or type(dictionary_data[0]) is str :
            ...
            gambar = "Default.jpg"
            penjelasan = "None"
            combine = {'kata': kata,'jenis' : jenis, 'kategori': "umum", 'pengucapan':pengucapan,'penjelasan':penjelasan,'pengucapan':pengucapan,'arti':arti, 'gambar':gambar}
