...
from get_random import generate_food
...
sum_word = len(food['word'].unique())
for i in range(sum_word): 
    word_to_lookup = generate_food()
    ...
    if dictionary_data is not None:
        if not dictionary_data or type(dictionary_data[0]) is str :
            ...
            gambar = "Default.jpg"
            penjelasan = "None"
            combine = {'kata': kata,'jenis' : jenis, 'kategori': "food_drink", 'pengucapan':pengucapan,'penjelasan':penjelasan,'pengucapan':pengucapan,'arti':arti, 'gambar':gambar}
...
file_path = "food-drink-bingbank.csv"
# Writing to CSV file
